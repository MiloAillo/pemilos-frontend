...
const SummaryRow = ({ candidate }: { candidate: SelectedCandidate }) => (
  <div className="flex gap-3 items-start rounded-xl border border-white/10 bg-white/[0.06] p-3">
    <img
    ...
    <div className="min-w-0">
      <p className="font-bodoni font-bold text-amber-100">
        {candidate.number}. {candidate.name}
        <span className="ml-2 text-xs font-sans font-semibold uppercase tracking-widest text-amber-200/60">
          {candidate.org}
        </span>
      </p>
      <p className="text-sm text-white/80 mt-1">{candidate.summary}</p>
      <a
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-[#1c1f2b] w-full max-w-lg h-fit p-6 rounded-2xl border border-amber-200/25 flex flex-col gap-4 shadow-2xl"
      >
        <div>
          <p className="font-bodoni font-black text-2xl text-amber-100 uppercase">
        ...
        <div className="flex flex-row gap-3">
          <button
            ...
            disabled={isSent}
            className="flex-1 text-center rounded-lg p-2.5 font-semibold border border-white/20 text-white hover:bg-white/10 transition disabled:opacity-40"
          >
            Sebentar
          </button>
          <button
            disabled={isSent}
            onClick={() => vote()}
            className="flex-1 flex items-center justify-center gap-2 text-center rounded-lg p-2.5 font-bodoni font-bold uppercase tracking-widest bg-amber-200 text-amber-950 hover:bg-amber-100 transition disabled:opacity-60"
          >
